...
import type { ProductAttributeItemType, UpdateProductAttributeRequestType } from "~~/shared/types/ApiResponseType";
...
const { updateProductAttribute } = useProductAttributeManagement();
...
watch(open, (val) => {
  if (!val || !props.attribute) return;
  formState.nameEn = String(props.attribute.name?.en || "");
  formState.nameKm = String(props.attribute.name?.km || "");
  values.value =
    props.attribute.values?.length
      ? props.attribute.values.map((v) => ({
          uuid: v.uuid,
          en: String(v.value?.en || ""),
          km: String(v.value?.km || ""),
        }))
      : [{ en: "", km: "" }];
});
...
async function onSubmit() {
  ...
  loading.value = true;
  try {
    const body: UpdateProductAttributeRequestType = {
      uuid: props.attribute.uuid,
      name: {},
      values: values.value
        .filter((v) => v.en || v.km)
        .map((v) => {
          ...
          return { uuid: v.uuid, value };
        }),
    };
    if (formState.nameEn) (body.name as any).en = formState.nameEn;
    if (formState.nameKm) (body.name as any).km = formState.nameKm;
    await updateProductAttribute(body);
    toast.add({ title: t("message.productAttributeUpdated"), color: "success" });
    open.value = false;
    emit("success");
  } catch {
...
<template>
  <UModal v-model:open="open">
    <template #content>
      <UCard>
        <template #header>
          <h3 class="text-lg font-semibold">{{ t("label.editProductAttribute") }}</h3>
        </template>
